...
function pico.captcha.create()
  ...
  local command = ("exec convert -size %dx%d "):format(width, height)

  -- Line mask
  command = command .. "xc: -draw \""
  local first = true
  ...
  while line_top < height do
    ...
    if not first then command = command .. " " else first = false end
    command = command .. ("rectangle 0,%d %d,%d"):format(line_top, width, line_bottom)

    line_top = line_bottom + random.int(min_line_size, max_line_size)
  end
  command = command .. "\" -write mpr:lines +delete "
  ...
  for i = 1, random.int(min_circle_count, max_circle_count) do
    ...
    command = command .. ("circle %d,%d %d,%d"):format(x, y, x + size, y + size)
  end
  command = command .. "\" -write mpr:circles +delete "
  ...
  command = command .. "-write mpr:text +delete "

  -- Text: alternative
  -- command = command .. ("xc: -font %s -pointsize %d -gravity center -draw \"text 0,0 '%s'\" -write mpr:text +delete "):format(
  --   font, font_size, text)
  ...
  -- Composite and Blur
  command = command .. "-compose Exclusion mpr:text mpr:lines -composite mpr:circles -composite -blur 1x1 jpg:-"
